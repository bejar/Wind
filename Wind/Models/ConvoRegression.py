...
def architectureConvDirRegression(idimensions, filters, kernel_size, strides, drop, nlayers, activation,
    ...
    if nlayers == 1:
        ...
        for i in range(1, nlayers - 1):
            ...
            if drop != 0:
                model.add(Dropout(rate=drop))
        model.add(Conv1D(filters, activation=activation, strides=strides,
                         kernel_size=kernel_size, activity_regularizer=act_regularizer,
                         kernel_regularizer=k_regularizer))
        if drop != 0:
            model.add(Dropout(rate=drop))
    model.add(Flatten())
...
def train_convdirregression_architecture(config, verbose, tboard, best, early, multi=1):
    ...
    for ahead in range(1, sahead + 1):
        ...
        strides = config['arch']['strides'][0]
        kernel_size = config['arch']['kernel_size'][0]
        ...
        print('DNM= %s, DS= %d, V= %d, LG= %d, AH= %d, KS= %s, ST= %s, LY= %d, FLT= %d, DR= %3.2f, AF= %s, '
