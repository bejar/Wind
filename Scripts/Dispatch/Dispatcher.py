...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp', default=None, help='Type of configs')
    ...
    while True:
        ...
        if len(lworkers) != 0:
            ...
            lsel = [c for c in col.find(query, limit=args.jpw * len(lworkers) * 10)]
        ...
        mres += len(lres)
        print(f'it {n} - served= {served} uploaded = {uploaded}  upnow = {len(lres)} mean = {mres/(n+1):3.2f} - {ctime()} ---', flush=True)
        n += 1
