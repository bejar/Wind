...
def train_regdir_architecture(config, impl, verbose):
    """
    ...
    for ahead in range(1, sahead + 1):
        ...
        model = architectureREG(idimensions=train_x.shape[1:], neurons=neurons, drop=drop, nlayers=nlayers,
                                activation=activation,
...
def train_MLP_regdir_architecture(config, verbose):
    """
...
if __name__ == '__main__':
    ...
    if config is not None:
        ...
        if config['arch']['mode'] == 'regdir':
            lresults = train_regdir_architecture(config, impl, verbose)
        elif config['arch']['mode'] == 'seq2seq':
            lresults = train_seq2seq_architecture(config, impl, verbose)
