...
def best_parameters(lexp, archtype, n):
    ...
    for i in range(n):
        ...
        if 'svm' in best[archtype][i]['arch']['mode']:
            ...
            print('NNEURD:', 0 if not 'neuronsD' in best[archtype][i]['arch'] else best[archtype][i]['arch']['neuronsD'])
            print('DROP:', best[archtype][i]['arch']['drop'])
            print('BI:', best[archtype][i]['arch']['bidirectional'] if 'bidirectional' in best[archtype][i]['arch'] else '')
